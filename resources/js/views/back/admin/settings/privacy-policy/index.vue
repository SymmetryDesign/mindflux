<template>
    <v-app-default-layout>
        <template v-slot:header>
            <h1 class="text-2xl font-semibold text-gray-900">{{ $trans('headings.privacy-policy') }}</h1>
        </template>
                        ...
                        <div class="bg-white shadow rounded-lg">
                            <div class="px-4 py-5 sm:px-6">
                                <h3 class="text-lg leading-6 font-medium text-gray-900">
                                    {{ $trans('headings.privacy-policy') }}
                                </h3>

                                <p class="mt-1 max-w-2xl text-sm leading-5 text-gray-500">
                                    {{ $trans('headings.subtexts.privacy-policy-description') }}
                                </p>
                            </div>
                                ...
                                <div class="form-group mb-6">
                                    <div class="flex items-start">
                                        ...
                                        <div class="pl-7 text-sm leading-5">
                                            <label for="published" class="font-medium text-gray-700">{{ $trans('labels.publish-privacy-policy') }}</label>
                                            <p class="text-gray-500">{{ $trans('labels.subtexts.publish-privacy-policy-warning') }}</p>
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    export default {
        metaInfo() {
            return {
                title: this.$trans('headings.privacy-policy')
            }
        },
        ...
        data() {
            return {
                form: new Form({
                    content: this.$page.privacy_policy,
        ...
        methods: {
            submit() {
                ...
                this.$inertia.put(route('admin:privacy-policy.update'), this.form.data())
