...
use App\Notifications\Task\TaskPhaseChanged;

class NotifyTaskPhaseChanged
{
    /**
     * Handle the event.
     *
    ...
    public function handle($event)
    {
        $event->task->project->teamMembers->each(function ($user) use ($event) {
            if ($user->preferences()->get('email_notifications.task_phase_changed', false) &&
                $event->task->project->isWatched($user)
            ) {
                $user->notify(new TaskPhaseChanged($event->task));
            }
        });
